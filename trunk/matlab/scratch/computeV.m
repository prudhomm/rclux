function [v, swamee ] = computeV( epsilon, L, D, prssDrop, rho, nu, v0 )
...
swameeJain = strcat(swameeJain,'+',num2str(Ke(D(1),D(2))+ Kc(D(2),D(3),pi)),')');
expr = strcat(swameeJain,'.*',num2str(rho),'.*v^2./2',num2str(-prssDrop));
